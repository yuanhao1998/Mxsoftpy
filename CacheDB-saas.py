...
class CacheDB(BaseDB):
    ...
    def check_data(self, key):
        ...
        """
        if key.find('dev') != 1:
            from utils.middleware.cache import device_type_cache,device_select_cache
            device_type_cache()
            device_select_cache()
        if key.find('Unit') != 1:
            from utils.middleware.cache import unit_cache
            unit_cache()
        if key == 'iot_json_cache':
            from utils.middleware.cache import iot_json_cache
            iot_json_cache()
        if key == 'target_json':
            from utils.middleware.cache import show_monitor_idx_cache
            show_monitor_idx_cache()
    ...
    def get(self, key: str) -> Any:
        ...
        try:
            ...
            if e.err_code in [28, 1000, 40]:  # 错误码28代表不存在此hash，此时返回None而不是报错
                self.check_data(key)
                return self.get(key)
            else:
